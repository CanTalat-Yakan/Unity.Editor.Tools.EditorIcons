...
namespace UnityEssentials
{
    public class EditorIcons : EditorWindow
    {
        ...
        {
            var all_icons = Icons.Where(x => GetIcon(x) != null);
            ...
            Icons = Icons.ToList().Concat(unique).ToArray();
        ...
        void InitializeIcons()
        {
            ...
            {
                GUIContent icon = GetIcon(Icons[i]);

                if (icon == null)
                {
                    _iconMissingNames.Add(Icons[i]);

                    continue;
                }

                icon.tooltip = Icons[i];
